...
class html_renderer extends \plugin_renderer_base {
    ...
    public function trainings_globals($userid, $from = NULL, $to = NULL, $height = 'large', &$stats) {
        ...
        if (is_null($stats)) {
            $stats = $compiler->get_user_globals($userid, $COURSE->id, $from, $to, 0);
        }
        ...
        $table->data[] = array((@$stats[$userid]->aratio + 0).' %',
    ...
    public function coverage_radar($userid, $from = NULL, $to = NULL, $title = '', $set = 'q') {
        ...
        $params = ['radar' => $radararg, 'headers' => $headersarg, 'fcolor' => $fcolor, 'bcolor' => $bcolor, 'width' => $w, 'height' => $h];
        $template->generatorurl = new moodle_url('/report/examtraining/gdgenerators/radargraph.php', $params);
        $params = ['radar' => $radarargmatched, 'headers' => $headersarg, 'fcolor' => $fcolormatched, 'bcolor' => $bcolormatched, 'width' => $w, 'height' => $h];
    ...
    public function trainings($userid, $from, $to) {
        ...
        if (!$stats = $compiler->get_weekly_globals($userid, $COURSE->id, $from, $to, 0)) {
    ...
    public function trainings_subcats($userid, $from = NULL, $to = NULL, $printmode = 'table') {
        ...
        if ($printmode == 'jqplot') {
            ...
            foreach ($stats as $catid => $stat) {
                ...
                $table->data[] = array($catname, $stats[$catid]->qratio. '% ('.$stats[$catid]->qmatched.'/'.$stats[$catid]->qcount.')');
            }
